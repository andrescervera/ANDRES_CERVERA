...
def run(argv=None):
    ...
    parser.add_argument('--runner', required=True, help='DataflowRunner')
    parser.add_argument('--job_name', required=True, help='Job name')
    parser.add_argument('--region', required=True, help='Region')
    parser.add_argument('--temp_location', required=True, help='Temporary location')
    ...
    with beam.Pipeline(options=options) as pipeline:
        ...
        _ = (
            formatted_data
            | 'Write to Fined or Non-Fined BigQuery' >> beam.ParDo(WriteToBigQuery(fined_table_spec))
        )

        nonfined_data = formatted_data | 'Filter non-fined data' >> beam.Filter(lambda x: not x["is_fined"])
        _ = nonfined_data | 'Write to Non-Fined BigQuery' >> beam.ParDo(WriteToBigQuery(nonfined_table_spec))
